...
<conditional>
Recall if the user prompt mentions monitor brightness, screen dimming, DDC, ddcutil, I2C display control, or VCP codes.
</conditional>

<fuzzy-match>
ddcutil, ddcutil-service, DDC/CI, VCP, busctl, i2c-dev, brightness script, ASUS VG328, /dev/i2c-6
</fuzzy-match>

<memory>
Controlling monitor brightness on the ASUS VG328 external display using DDC/CI over I2C, with ddcutil-service as the D-Bus backend and a custom brightness wrapper script.

## Why not brightnessctl?

`brightnessctl` only works with kernel backlight drivers (laptop panels). This system has no `/sys/class/backlight` device — only keyboard/NIC LEDs in `/sys/class/leds`. External monitors like the ASUS VG328 don't expose a kernel backlight interface, so DDC/CI is the only option.

## How DDC/CI works

External monitors use **DDC/CI** (Display Data Channel Command Interface) over the **I2C** bus to control settings. The protocol follows the **MCCS** (Monitor Control Command Set) standard, where each setting is identified by a **VCP** (Virtual Control Panel) feature code.
...
The `i2c-dev` module must be loaded for `/dev/i2c-*` devices to exist.
...
Keeps the I2C file descriptor open persistently, avoiding the re-initialization overhead that makes raw ddcutil slow. It is D-Bus activated — starts on first call, no systemd service to manage.
