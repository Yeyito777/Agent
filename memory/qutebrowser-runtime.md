...
Qutebrowser runs fully containerized via `--basedir`. All runtime state lives in a single directory instead of being scattered across `~/.config`, `~/.local/share`, and `~/.cache`.
...
## config.py
- Loads `whale` theme module (lives in `config/whale/`)
- Sources `permissions.py` for site-specific permissions (auto-generated by qutebrowser)
- CSS overrides in `config/cssoverrides/`
