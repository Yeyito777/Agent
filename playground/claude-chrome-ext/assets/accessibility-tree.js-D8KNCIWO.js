        ...
        let h = function (e) {
          ...
          g = function (e) {
            ...
            if ("select" === t) {
              var r = e,
                i =
                  r.querySelector("option[selected]") ||
                  r.options[r.selectedIndex];
              if (i && i.textContent) return i.textContent.trim();
            }
            ...
            if ("input" === t) {
              ...
              if ("submit" === c && f && f.trim()) return f.trim();
            ...
            if (t.match(/^h[1-6]$/)) {
              var s = e.textContent;
              if (s && s.trim()) return s.trim().substring(0, 100);
            }
            if ("img" === t) return "";
            for (var p = "", w = 0; w < e.childNodes.length; w++) {
              var b = e.childNodes[w];
              b.nodeType === Node.TEXT_NODE && (p += b.textContent);
            }
